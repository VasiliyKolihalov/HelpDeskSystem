...
public class MessagesRepository : IMessagesRepository
{
    ...
    {
        const string query = "insert into Messages values (@Id, @SupportTicketId, @UserId, @Content, @DateTime)";
        await using DbConnection connection = _dbContext.CreateConnection();
        await connection.ExecuteTransactionAsync(async transaction =>
        {
            var param = new { item.Id, item.SupportTicketId, UserId = item.User.Id, item.Content, item.DateTime };
